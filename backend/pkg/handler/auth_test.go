	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockAuthorization, user signInInput) {
				s.EXPECT().GenerateToken(user.Password, user.Email).Return(augventure.Author{
					Name:     "",
					Username: "",
					Email:    "test@mail.ru",
					PfpUrl:   "",
			...
			expectedStatusCode:  200,
			expectedRequestBody: `{"user":{"name":"","username":"","email":"test@mail.ru","pfp_url":"","bio":""},"token":"1"}`,
		},
		{
			name:                "Empty Fields",
